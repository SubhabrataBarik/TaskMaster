var RENDERER = {
    ...
    INIT_RAIN_DROP_COUNT: 800,   // optimized from 1500
    RAIN_DROP_COUNT: 6,
    HUMAN_COUNT: 3,

    COLOR: 'rgba(192,192,192,0.18)', // silver trail background
    ...
    createRainDrops: function (count, toInit) {
        for (var i = 0; i < count; i++) {
            this.rainDrops.push(
                new RAIN_DROP(this.width, this.height, toInit, this)
            );
        }
    },
...
RAIN_DROP.prototype = {
    ...
    VELOCITY_RANGE: { min: -0.6, max: -0.4 },
    VELOCITY_RATE: 3,

    LENGTH_RATE: 8,   // smaller drop size
    ACCELARATION_RATE: 0.01,
    VERTICAL_OFFSET_RATE: 0.01,

    FRONT_THRESHOLD: 0.8,
    REFLECTION_RADIUS_RATE: 0.52,

    COLOR: 'rgba(255,255,255,0.85)', // white rain
    ...
    init: function () {
        ...
        this.y = (this.toInit
            ? this.renderer.getRandomValue({ min: 0, max: this.height })
            : 0) - this.offset;
    ...
    render: function (context, toFront) {
        ...
        if (
            this.y >=
            this.height *
            (1 - (1 - this.scale) * this.THRESHOLD_RATE) -
            this.offset
        ) {

            context.lineWidth = 2;
            context.globalAlpha =
                (1 - this.radius / this.length / this.RADIUS_RATE) * 0.5;
...
HUMAN.prototype = {
    ...
    VELOCITY_OFFSET: { min: 0.2, max: 0.5 },
    SHADOW_SCALE: -0.6,
    ...
    setParameters: function (toInit) {
        ...
        var position =
            this.renderer.getRandomValue(
                this.VERTICAL_OFFSET_RATE_RANGE
            ) * this.height;

        var verticalPosition =
            this.height * this.VERTICAL_OFFSET_RATE_RANGE.max;

        this.rate =
            (10 * (verticalPosition - position)) / verticalPosition | 0;

        this.y = this.height - position;

        this.vx =
            this.renderer.getRandomValue(this.VELOCITY_OFFSET) *
            this.direction;
    ...
    renderSubstance: function (context) {
        ...
        this.renderHuman(context, 'rgba(60,60,60,0.9)');
        context.restore();
        ...
        if (
            (this.x < -this.HORIZONTAL_OFFSET && this.direction < 0) ||
            (this.x > this.width + this.HORIZONTAL_OFFSET &&
                this.direction > 0)
        ) {
    ...
    renderShadow: function (context) {
        ...
        this.renderHuman(context, 'rgba(255,255,255,0.35)');
        context.restore();
    },
    
    

    renderHuman(context, color) {
